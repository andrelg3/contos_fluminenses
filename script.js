...
(function() {
  ...
  /* ------------------------------------------------------------------
     CLASSES — Leitura e escrita central via Google Apps Script
     Todas as máquinas leem e gravam no mesmo lugar.
     ------------------------------------------------------------------ */
  function fetchClassesFromGoogleSheets() {
    const scriptUrl = getGoogleSheetsURL();
    if (!scriptUrl) { updateClassDropdowns(); return; }
    ...
    fetch(scriptUrl + '?action=getClasses')
      ...
      .catch(() => {
        // Offline: usa padrão já definido no STATE
        updateClassDropdowns();
      });
  }
  ...
  function syncClassesToGoogleSheets() {
    ...
    fetch(scriptUrl, {
      ...
      body: JSON.stringify({ action: 'saveClasses', classes: STATE.classesList })
    }).catch(err => console.log('Classes sync notice:', err));
  }
  ...
  function updateClassDropdowns() {
    ...
    if (DOM.classesTagsList) {
      ...
      STATE.classesList.forEach(cls => {
        ...
        tag.innerHTML = `
          <span>🏫 ${cls}</span>
          <button class="class-tag-delete" onclick="window.CF_GAME.deleteClass('${cls}')" title="Excluir turma ${cls}">&times;</button>
        `;
  ...
  function handleAddClass(e) {
    ...
    syncClassesToGoogleSheets(); // Salva globalmente no Apps Script
    updateClassDropdowns();
    DOM.inputNewClassName.value = '';
    showToast(`Turma "${name}" adicionada! Visível em todas as máquinas em instantes.`);
  }
  ...
  function deleteClass(className) {
    ...
    if (confirm(`Tem certeza que deseja excluir a turma "${className}"?\n\nAlunos cadastrados nesta turma permanecerão no ranking.`)) {
      STATE.classesList = STATE.classesList.filter(c => c !== className);
      syncClassesToGoogleSheets(); // Salva globalmente no Apps Script
      updateClassDropdowns();
      showToast(`Turma "${className}" removida! Alteração refletida em todas as máquinas.`);
    }
  }
  ...
  function syncStudentToGoogleSheets(student) {
    ...
    }).catch(err => console.log('Offline/Sync notice:', err));
  }
  ...
  function fetchRankingFromGoogleSheets() {
    ...
    fetch(scriptUrl + '?action=getRanking')
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          data.forEach(remoteStd => {
            const idx = STATE.studentsList.findIndex(s => s.id === remoteStd.id || (s.name === remoteStd.name && s.studentClass === remoteStd.studentClass));
            if (idx !== -1) {
              if ((remoteStd.xp || 0) > STATE.studentsList[idx].xp) {
                STATE.studentsList[idx] = { ...STATE.studentsList[idx], ...remoteStd };
              }
            } else {
              STATE.studentsList.push(remoteStd);
            }
          });
          saveToStorage();
          renderProfAnalytics();
      ...
      .catch(err => console.log('Notice fetching sheets:', err));
  }

  function saveToStorage() {
    try {
      if (STATE.currentUser) {
        const idx = STATE.studentsList.findIndex(s => s.id === STATE.currentUser.id);
        if (idx !== -1) {
  ...
  function handleLogin(e) {
    ...
    const existing = STATE.studentsList.find(
      s => s.name.toLowerCase() === name.toLowerCase() && s.studentClass === studentClass
    );
  ...
  function onUserLoggedIn() {
    ...
    const firstName = STATE.currentUser.name.trim().split(' ')[0];
  ...
  function renderRankingModal() {
    ...
    top3.forEach((std, idx) => {
      ...
      item.innerHTML = `
        ...
        <div class="podium-score">${std.xp} XP</div>
    ...
    list.forEach((std, idx) => {
      ...
      const completionPercent = Math.round((std.completedStories.length / STORIES.length) * 100);

      tr.innerHTML = `
        <td><strong>#${idx + 1}</strong></td>
        <td>${std.name} ${STATE.currentUser && std.id === STATE.currentUser.id ? '⭐ (Você)' : ''}</td>
        <td><span class="story-tag-pill">${std.studentClass}</span></td>
        <td><strong style="color:var(--primary-gold);">${std.xp} XP</strong></td>
        <td>${std.completedStories.length}/7 Contos</td>
  ...
  function resetAllData() {
    if (confirm("Tem certeza que deseja zerar a lista de todos os alunos e o ranking? Esta ação é irreversível.")) {
      STATE.studentsList = [];
      STATE.currentUser = null;
  ...
  function configureGoogleSheetsURL() {
    ...
    const url = prompt("Cole aqui a URL da sua API do Google Apps Script (Web App):\nExemplo: https://script.google.com/macros/s/AKfycbx.../exec", current);
    if (url !== null) {
      localStorage.setItem('CF_GOOGLE_SHEETS_URL', url.trim());
      if (url.trim()) {
        showToast("✓ Integracao com Google Sheets configurada!");
        fetchRankingFromGoogleSheets();
      } else {
        showToast("Integracao com Google Sheets desativada.");
      }
    }
  }
  ...
  function renderProfAnalytics() {
    ...
    list.forEach(s => {
      const score10 = (s.completedStories.length / STORIES.length) * 10;
      totalScoreSum += score10;
      if (s.completedStories.length >= STORIES.length) completedAllCount++;
    });
    ...
    list.forEach(std => {
      const nota10 = ((std.completedStories.length / STORIES.length) * 10).toFixed(1);
      const tr = document.createElement('tr');
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn btn-outline btn-sm" onclick="alert('Aluno: ${std.name}\\nTurma: ${std.studentClass}\\nXP: ${std.xp}\\nContos: ${std.completedStories.join(', ') || 'Nenhum'}')">
            🔍 Detalhes
          </button>
        </td>
      `;
  ...
  function exportCSV() {
    ...
    list.forEach(s => {
      const nota10 = ((s.completedStories.length / STORIES.length) * 10).toFixed(1);
      csvContent += `"${s.id}","${s.name}","${s.studentClass}",${s.xp},${nota10},"${s.completedStories.length}/7","${s.bossPassed ? 'Aprovado' : 'Pendente'}"\n`;
    });
  ...
  function setupEventListeners() {
    ...
    const btnConfigGoogleSheets = document.getElementById('btnConfigGoogleSheets');
    if (btnConfigGoogleSheets) {
      btnConfigGoogleSheets.addEventListener('click', configureGoogleSheetsURL);
    }
