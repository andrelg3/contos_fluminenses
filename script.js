...
(function() {
  ...
  function loadClassesFromStorage() {
    try {
      const data = localStorage.getItem('CF_CLASSES_LIST');
      if (data) {
        const parsed = JSON.parse(data);
        if (Array.isArray(parsed) && parsed.length > 0) {
          STATE.classesList = parsed;
        }
      }
    } catch (e) {
  ...
  function handleAddClass(e) {
    ...
    showToast(`Turma "${name}" adicionada com sucesso!`);
  }
  ...
  function deleteClass(className) {
    ...
    if (confirm(`Tem certeza que deseja excluir a turma "${className}"?\n\nAlunos cadastrados nesta turma permanecerão no ranking.`)) {
      ...
      saveClassesToStorage();
      updateClassDropdowns();
      showToast(`Turma "${className}" removida com sucesso!`);
    }
  }
